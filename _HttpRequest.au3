;Thanks [@ProAndy] [Trancexx] autoitscript.com
;Thanks [ozmike - Json Obj] autoitscript.com
...
Global $g___WinHttp = 'winhttp.dll'
...
;------------------------------------------------------------------------------------
Global $g___OO_Json, $g___OO_LoadLibrary = '', $g___OO_JsonInit = 0
;------------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------------
Global Const $g___defUserAgentW = 'Mozilla/5.0 (Windows NT 6.1; rv:53.0) Gecko/20100101 Firefox/53.0'
...
;-----------------------------------------------------------------------------------------
;~ If Not @Compiled and @OSVersion = 'WIN_10' Then
;~ 	Local $pathWH = StringTrimRight(@AutoItExe, 11) & 'winhttp.dll'
;~ 	If Not FileExists($pathWH) Then
;~ 		InetGet('http://posttestserver.com/files/2017/09/30/f_12.16.03444506166', $pathWH, 8)
;~ 		If Not @error Then $g___WinHttp = $pathWH
;~ 	Else
;~ 		$g___WinHttp = $pathWH
;~ 	EndIf
;~ EndIf
;-----------------------------------------------------------------------------------------
Global $dll_WinHttp = DllOpen($g___WinHttp)
DllOpen($g___WinHttp)
...
Func _HttpRequest($iReturn, $sURL, $sData2Send = '', $sCookie = '', $sReferer = '', $sAdditional_Headers = '', $sOVerb = '', $CallBackFunc_Progress = '')
	...
	If $sData2Send Then
		If Not $g___Boundary Then
			$g___Boundary = StringRegExp($sData2Send, '^(-{3,}\w+)\R', 1)
			If Not @error Then $g___Boundary = $g___Boundary[0]
		EndIf
		...
		Else
			If Not $vContentType Then
				If StringRegExp($sData2Send, '^[\s\t]*?[\{\[][\s\t]*?[''"]?') Then
					...
					$sData2Send = StringToBinary($sData2Send, 4)
				Else
	...
	If _WinHttpSendRequest2($g___hRequest, $sAdditional_Headers & $vContentType, $sData2Send, $CallBackFunc_Progress) = 0 Then
		Switch @error
			...
			Case Else
				$iError = 3
				__HttpRequest_ErrNotify('_HttpRequest', 'Send Request Failed')
		EndSwitch
	...
	Else
		If _WinHttpReceiveResponse2($g___hRequest) Then
			...
			EndSwitch
		Else
			__HttpRequest_ErrNotify('_HttpRequest', 'Did Not Receive Response from Server')
			If $aRetMode[0] = 8 Then $g___retData[0] = $g___ServerIP
			$iError = 6
		EndIf
	EndIf
...
Func _HttpRequest_CreateDataForm($a_FormItems) ;thêm dấu $ để nhận biết đó là 1 file, thêm dấu ~ để chuyển Unicode sang Ansi
	...
	Local $sData2Send = $g___Boundary & @CRLF, $vValue, $iUploadFile = 0
	;------------------------------------------------------------------------------------------
	...
	;------------------------------------------------------------------------------------------
	Return $iUploadFile = 0 ? StringToBinary(StringTrimRight($sData2Send, 2) & '--', 4) : StringTrimRight($sData2Send, 2) & '--'
EndFunc
...
;$iReturn = 0: String, 1: Array, 2:Struct, 3:Struct+String
;$iInputType = 0:hidden, 1:text, 2: cả 2
Func _HttpRequest_SearchHiddenValues($iSourceHtml_or_URL, $iKeySearch = '', $iReturn = 1, $iInputType = 0)
	Local $vCookie = ''
	If $iReturn = Default Then $iReturn = True
	...
	If StringRegExp($iSourceHtml_or_URL, '^https?://') And Not StringRegExp($iSourceHtml_or_URL, '[\r\n]') Then
		$iSourceHtml_or_URL = _HttpRequest(4, $iSourceHtml_or_URL)
		If @error Then Return SetError(-1, __HttpRequest_ErrNotify('_HttpRequest_SearchHiddenValues', 'Get Source Failed'), '')
		Local $iErrNoti = $g___ErrorNotify
		$g___ErrorNotify = False
		$vCookie = _GetCookie($iSourceHtml_or_URL[0])
		$g___ErrorNotify = $iErrNoti
		If @error Then $vCookie = ''
		$iSourceHtml_or_URL = $iSourceHtml_or_URL[1]
	EndIf
	Local $aInput = StringRegExp($iSourceHtml_or_URL, '(?i)<input (.*?)\/?>', 3)
	If @error Then Return SetError(1, __HttpRequest_ErrNotify('_HttpRequest_SearchHiddenValues', 'No Hidden or Text Input #1'), '')
	Local $uBound = UBound($aInput), $iCounter = 0, $aRet[0][3], $sRet = '', $tRet = ''
	Local $vType, $vName, $vValue
	For $i = 0 To $uBound - 1
		$vType = StringRegExp($aInput[$i], '(?i)type\s*?=\s*?["''](hidden|text)[''"]', 1)
		If @error Or ($iInputType = 0 and $vType[0] <> 'hidden') Or ($iInputType = 1 and $vType <> 'text') Then ContinueLoop
		$vName = StringRegExp($aInput[$i], '(?i)name\s*?=\s*?[''"](\w+)[''"]', 1)
		If @error Then ContinueLoop
		If IsArray($iKeySearch) Then
			For $k = 1 To $iKeySearch[0]
				If $vName[0] = $iKeySearch[$k] Then ExitLoop
			Next
			If $k > $iKeySearch[0] Then ContinueLoop
		EndIf
		$vValue = StringRegExp($aInput[$i], '(?i)value\s*?=\s*?[''"](.*?)[''"]', 1)
		If @error Then ContinueLoop
		$sRet &= $vName[0] & '=' & $vValue[0] & '&'
		If $iReturn > 1 Then $tRet &= 'wchar ' & StringRegExpReplace($vName[0], '[^\w]', '') & '[' & StringLen($vName[0] & $vValue[0]) + 1 & '];'
		ReDim $aRet[$iCounter + 1][3]
		$aRet[$iCounter][0] = $vName[0]
		$aRet[$iCounter][1] = $vValue[0]
		$aRet[$iCounter][2] = $vType[0]
		$iCounter += 1
	Next
	If $iReturn = 0 Then
		If $sRet = '' Then Return SetError(3, __HttpRequest_ErrNotify('_HttpRequest_SearchHiddenValues', 'No Hidden or Text Input #2'), '')
		Return StringTrimRight($sRet, 1)
	Else
		If UBound($aRet) = 0 Then Return SetError(2, __HttpRequest_ErrNotify('_HttpRequest_SearchHiddenValues', 'No Hidden or Text Input #3'), '')
		If $iReturn = 1 Then Return $aRet
		If $iReturn > 1 Then
			$tRet = DllStructCreate($tRet & 'wchar Cookie[' & StringLen($vCookie) + 1 & ']')
			For $i = 0 To UBound($aRet) - 1
				DllStructSetData($tRet, $i + 1, $iReturn = 2 ? $aRet[$i][1] : $aRet[$i][0] & '=' & $aRet[$i][1])
			Next
			DllStructSetData($tRet, 'Cookie', $vCookie)
			Return $tRet
		EndIf
	EndIf
EndFunc
...
Func _HttpRequest_BypassCloudflare($URL_Domain, $iTimeout = Default)
	...
	For $i = 0 To UBound($number_jschl_math) - 1 Step 2
		...
		Switch StringStripWS($number_jschl_math[$i], 8)
			Case '+'
				$jschl_answer += $char2num
			Case '-'
				$jschl_answer -= $char2num
			Case '*'
				$jschl_answer *= $char2num
...
Func _B64Encode($binaryData, $iLinebreak = 0, $safeB64 = False, $iRunByMachineCode = False, $iGZCompress = False)
	...
	Else
		...
		If @error Or Not $a_Call[0] Then Return SetError(2, __HttpRequest_ErrNotify('_B64Encode', 'CryptBinaryToString Failed'), $binaryData)
		...
		If @error Or Not $a_Call[0] Then Return SetError(3, __HttpRequest_ErrNotify('_B64Encode', 'CryptBinaryToString Failed'), $binaryData)
...
Func _B64Decode($base64Data, $iRunByMachineCode = False, $iGZUnCompress = False)
	...
	;----------------------------------------------------------------------------------------
	If Not $iRunByMachineCode Then
		If Mod(StringLen($base64Data), 2) Then Return SetError(2, __HttpRequest_ErrNotify('_B64Decode', 2), $base64Data)
	...
	Else
		...
		If @error Or Not $a_Call[0] Then Return SetError(3, __HttpRequest_ErrNotify('_B64Decode', 'CryptBinaryToString Failed'), $base64Data)
		...
		If @error Or Not $a_Call[0] Then Return SetError(4, __HttpRequest_ErrNotify('_B64Decode', 'CryptBinaryToString Failed'), $base64Data)
...
#Region <FUNC đã đổi tên và sẽ bị loại bỏ ở phiên bản sau>
	...
	Func _SciTE_TextSplit($nameVar, $nCharPerLine = 101)
...
#Region <Chạy code javascript, php>
	Func __JS_Beautify($jsCode)
		$jsCode = _HttpRequest(2, 'https://javascriptbeautifier.com/compress', 'js_code=' & _URIEncode($jsCode) & '&js_code_result=&beautify=true')
	...
	Func __JS_Compress($jsCode)
		$jsCode = _HttpRequest(2, 'https://javascript-minifier.com/raw', 'input=' & _URIEncode($jsCode))
		If @error Or $jsCode = '' Or StringInStr($jsCode, '// Error:', 0, 1, 1, 10) Then Return SetError(1, '', $jsCode)
		Return $jsCode
	EndFunc
	...
	Func _JS_ToStringAu3($jsMode = 0) ;0: Chỉ chuyển sang string, 1: Compress trước khi chuyển, 2: beautify trước khi chuyển
		...
		Switch $jsMode
			Case 1
				$jsCode = __JS_Compress($jsCode)
			Case 2
				$jsCode = __JS_Beautify($jsCode)
		EndSwitch
	...
	Func _JS_Execute($LibraryJS, $sCodeJS, $Name_Var_Return_Val, $ModeIE = False, $PathTempLibJS = Default)
		...
		EndIf
		;-----------------------------------------------------------------------------------------------------------
		$sCodeJS = '<script>' & @CRLF & $sCodeJS & ';' & @CRLF & '</script>' & @CRLF
		$sCodeJS &= '<script>document.write(' & $Name_Var_Return_Val & ');</script>'
		$sCodeJS = '<script>' & @CRLF & $sLibraryJS & ';' & @CRLF & '</script>' & @CRLF & $sCodeJS
		If $ModeIE Then
...
#Region <HTML Entities>
	...
	Func __HTML_Entities_Init()
		If $g___aChrEnt == '' Then
			Local $sChrEnt = ''
			$sChrEnt &= '0x2271756F747C2761706F737C26616D707C3C6C747C3E67747CA06E6273707CA1696578636C7CA263656E747CA3706F756E64'
			$sChrEnt &= '7CA463757272656E7CA579656E7CA66272766261727CA7736563747CA8756D6C7CA9636F70797CAA6F7264667CAB6C6171756'
			$sChrEnt &= 'F7CAC6E6F747CAE7265677CAF6D6163727CB1706C75736D6E7CB461637574657CB56D6963726F7CB6706172617CB76D696464'
			$sChrEnt &= '6F747CB8636564696C7CB9737570317CB2737570327CB3737570337CBC6672616331347CBD6672616331327CBE6672616333347'
			$sChrEnt &= 'CBF6971756573747CDF737A6C69677CF76469766964657C88636972637C9874696C64657C3F7369676D61667C3F746865746173'
			$sChrEnt &= '796D7C3F75707369687C3F7069767CB06465677CD774696D65737CBA6F72646D7CBB726171756F7CAD7368797CE06167726176'
			$sChrEnt &= '657CE16161637574657CE261636972637CE36174696C64657CE461756D6C7CE56172696E677CE661656C69677CE763636564696C'
			$sChrEnt &= '7CEA65636972637CEB65756D6C7CEC6967726176657CED6961637574657CEE69636972637CEF69756D6C7CF06574687CF16E746'
			$sChrEnt &= '96C64657CF26F67726176657CF36F61637574657CF46F636972637CF56F74696C64657CF66F756D6C7CF86F736C6173687CF975'
			$sChrEnt &= '67726176657CFA7561637574657CFB75636972637CFC75756D6C7C9C6F656C69677C9A736361726F6E7C83666E6F667C61616C'
			$sChrEnt &= '7068617CDF626574617C3F67616D6D617C6464656C74617C65657073696C6F6E7C3F696F74617C3F6B617070617C3F6C616D62'
			$sChrEnt &= '64617CB56D757C3F6E757C3F78697C3F6F6D6963726F6E7C7070697C737369676D617C747461757C3F757073696C6F6E7C66706'
			$sChrEnt &= '8697C3F6368697C3F7073697C3F6F6D6567617CFE74686F726E7C3F72686F7C3F7A6574617C3F6574617C3F74686574617CE865'
			$sChrEnt &= '67726176657CE96561637574657CFD7961637574657CFF79756D6C'
			$g___aChrEnt = StringSplit($sChrEnt, '|', 2)
		EndIf
		...
		For $i = 0 To UBound($g___aChrEnt) - 1
			$aChrEnt = StringRegExp($g___aChrEnt[$i], '^(.)(.)(.+)$', 3)
			If $i > 45 Then $g___oDicEntity.add(StringUpper($aChrEnt[1]) & $aChrEnt[2], StringUpper($aChrEnt[0]))
			$g___oDicEntity.add($aChrEnt[1] & $aChrEnt[2], $aChrEnt[0])
		Next
...
#Region <JSON OBJECT>
	Func _JSON_Object_Init($jsonDefault = "{}")
		; Author: ozmike
		; Link: https://www.autoitscript.com/forum/topic/156794-oo_jsonudf-jsonpath-oo-using-javascript-in-auto-it
		...
		If $g___OO_LoadLibrary == '' Then
			$g___OO_LoadLibrary &= 'function buildParamlist(t){var e=" p0";t=t||1;for(var r=1;r<t;r++)e=e+" , p"+r;return e}function myt'
			$g___OO_LoadLibrary &= 'est(){return"mytest."}function jsonPath(obj,expr,arg){var P={resultType:arg&&arg.resultType||"VALUE"'
			$g___OO_LoadLibrary &= ',result:[],normalize:function(t){var e=[];return t.replace(/[\[''](\??\(.*?\))[\]'']/g,function(t,r)'
			$g___OO_LoadLibrary &= '{return"[#"+(e.push(r)-1)+"]"}).replace(/''?\.''?|\[''?/g,";").replace(/;;;|;;/g,";..;").replace(/;$'
			$g___OO_LoadLibrary &= '|''?\]|''$/g,"").replace(/#([0-9]+)/g,function(t,r){return e[r]})},asPath:function(t){for(var e=t.sp'
			$g___OO_LoadLibrary &= 'lit(";"),r="$",n=1,o=e.length;n<o;n++)r+=/^[0-9*]+$/.test(e[n])?"["+e[n]+"]":"[''"+e[n]+"'']";return'
			$g___OO_LoadLibrary &= ' r},store:function(t,e){return t&&(P.result[P.result.length]="PATH"==P.resultType?P.asPath(t):e),!!t'
			$g___OO_LoadLibrary &= '},trace:function(t,e,r){if(t){var n=t.split(";"),o=n.shift();if(n=n.join(";"),e&&e.hasOwnProperty(o)'
			$g___OO_LoadLibrary &= ')P.trace(n,e[o],r+";"+o);else if("*"===o)P.walk(o,n,e,r,function(t,e,r,n,o){P.trace(t+";"+r,n,o)});e'
			$g___OO_LoadLibrary &= 'lse if(".."===o)P.trace(n,e,r),P.walk(o,n,e,r,function(t,e,r,n,o){"object"==typeof n[t]&&P.trace("..'
			$g___OO_LoadLibrary &= ';"+r,n[t],o+";"+t)});else if(/,/.test(o))for(var a=o.split(/''?,''?/),u=0,p=a.length;u<p;u++)P.trace'
			$g___OO_LoadLibrary &= '(a[u]+";"+n,e,r);else/^\(.*?\)$/.test(o)?P.trace(P.eval(o,e,r.substr(r.lastIndexOf(";")+1))+";"+n,e,'
			$g___OO_LoadLibrary &= 'r):/^\?\(.*?\)$/.test(o)?P.walk(o,n,e,r,function(t,e,r,n,o){P.eval(e.replace(/^\?\((.*?)\)$/,"$1"),n'
			$g___OO_LoadLibrary &= '[t],t)&&P.trace(t+";"+r,n,o)}):/^(-?[0-9]*):(-?[0-9]*):?([0-9]*)$/.test(o)&&P.slice(o,n,e,r)}else P.'
			$g___OO_LoadLibrary &= 'store(r,e)},walk:function(t,e,r,n,o){if(r instanceof Array)for(var a=0,u=r.length;a<u;a++)a in r&&o('
			$g___OO_LoadLibrary &= 'a,t,e,r,n);else if("object"==typeof r)for(var p in r)r.hasOwnProperty(p)&&o(p,t,e,r,n)},slice:functi'
			$g___OO_LoadLibrary &= 'on(t,e,r,n){if(r instanceof Array){var o=r.length,a=0,u=o,p=1;t.replace(/^(-?[0-9]*):(-?[0-9]*):?(-?'
			$g___OO_LoadLibrary &= '[0-9]*)$/g,function(t,e,r,n){a=parseInt(e||a),u=parseInt(r||u),p=parseInt(n||p)}),a=a<0?Math.max(0,a'
			$g___OO_LoadLibrary &= '+o):Math.min(o,a),u=u<0?Math.max(0,u+o):Math.min(o,u);for(var c=a;c<u;c+=p)P.trace(c+";"+e,r,n)}},ev'
			$g___OO_LoadLibrary &= 'al:function(x,_v,_vname){try{return $&&_v&&eval(x.replace(/@/g,"_v"))}catch(t){throw new SyntaxError'
			$g___OO_LoadLibrary &= '("jsonPath: "+t.message+": "+x.replace(/@/g,"_v").replace(/\^/g,"_a"))}}},$=obj;if(expr&&obj&&("VALU'
			$g___OO_LoadLibrary &= 'E"==P.resultType||"PATH"==P.resultType))return P.trace(P.normalize(expr).replace(/^\$;/,""),obj,"$")'
			$g___OO_LoadLibrary &= ',!!P.result.length&&P.result}Object.prototype.propAdd=function(prop,val){eval("this."+prop+"="+val)}'
			$g___OO_LoadLibrary &= ',Object.prototype.methAdd=function(meth,def){eval("this."+meth+"= new "+def)},Object.prototype.jsFun'
			$g___OO_LoadLibrary &= 'Add=function(funname,numParams,objectTypeName){var x=buildParamlist(numParams);return objectType'
			$g___OO_LoadLibrary &= 'Name=objectTypeName||"Object",eval(objectTypeName+".prototype."+funname+" = function("+x+") { return'
			$g___OO_LoadLibrary &= ' "+funname+"("+x+"); }")},Object.prototype.protoAdd=function(methName,jsFunction,objectTypeName){obj'
			$g___OO_LoadLibrary &= 'ectTypeName=objectTypeName||"Object",eval(objectTypeName+".prototype."+methName+"="+jsFunction)},'
			$g___OO_LoadLibrary &= 'Object.keys||(Object.keys=function(){"use strict";var t=Object.prototype.hasOwnProperty,e=!{toString:null}.'
			$g___OO_LoadLibrary &= 'propertyIsEnumerable("toString"),r=["toString","toLocaleString","valueOf","hasOwnProperty","isProtot'
			$g___OO_LoadLibrary &= 'ypeOf","propertyIsEnumerable","constructor"],n=r.length;return function(o){if("object"!=typeof o&&("'
			$g___OO_LoadLibrary &= 'function"!=typeof o||null===o))throw new TypeError("Object.keys called on non-object");var a,u,p=[];'
			$g___OO_LoadLibrary &= 'for(a in o)t.call(o,a)&&p.push(a);if(e)for(u=0;u<n;u++)t.call(o,r[u])&&p.push(r[u]);return p}}()),Ob'
			$g___OO_LoadLibrary &= 'ject.prototype.objGet=function(s){return eval(s)},REM="To get an obj $obj.objGet(''JSON'')  ",Array.'
			$g___OO_LoadLibrary &= 'prototype.item=function(t){return this[t]},REM=" so that arrays can work in AutoIT obj.item(0)",Obje'
			$g___OO_LoadLibrary &= 'ct.prototype.item=function(t){return this[t]},REM="so that dynamic key values be obtained eg. obj.it'
			$g___OO_LoadLibrary &= 'em(''name'' ) or just obj.surname ",Object.prototype.keys=function(){if("object"==typeof this)return'
			$g___OO_LoadLibrary &= ' Object.keys(this)},REM="To get an obj json properties ",Object.prototype.keys=function(t){if("objec'
			$g___OO_LoadLibrary &= 't"==typeof t)return Object.keys(t)},REM="To get an obj json properties  - recommended usage ",Object'
			$g___OO_LoadLibrary &= '.prototype.arrayAdd=function(t,e){this[t]=e},REM="Add item to array  ",Object.prototype.arrayDel=fun'
			$g___OO_LoadLibrary &= 'ction(t){this.splice(t,1)},REM="Del item to array  ",Object.prototype.isArray=function(){return this'
			$g___OO_LoadLibrary &= '.constructor==Array},REM="Is object an array  ",Object.prototype.type=function(){return typeof this}'
			$g___OO_LoadLibrary &= ',REM="To get an obj type - not recommended ",Object.prototype.type=function(t){return typeof t},REM='
			$g___OO_LoadLibrary &= '"To get an obj type - recommended usage ";var JSON=new Object;JSON.jsonPath=function(t,e,r){return j'
			$g___OO_LoadLibrary &= 'sonPath(t,e,r)},"object"!=typeof JSON&&(JSON={}),function(){"use strict";function f(t){return t<10?"'
			$g___OO_LoadLibrary &= '0"+t:t}function quote(t){return escapable.lastIndex=0,escapable.test(t)?''"''+t.replace(escapable,fu'
			$g___OO_LoadLibrary &= 'nction(t){var e=meta[t];return"string"==typeof e?e:"\\u"+("0000"+t.charCodeAt(0).toString(16)).slice'
			$g___OO_LoadLibrary &= '(-4)})+''"'':''"''+t+''"''}function str(t,e){var r,n,o,a,u,p=gap,c=e[t];switch(c&&"object"==typeof c'
			$g___OO_LoadLibrary &= '&&"function"==typeof c.toJSON&&(c=c.toJSON(t)),"function"==typeof rep&&(c=rep.call(e,t,c)),typeof c)'
			$g___OO_LoadLibrary &= '{case"string":return quote(c);case"number":return isFinite(c)?String(c):"null";case"boolean":case"nu'
			$g___OO_LoadLibrary &= 'll":return String(c);case"object":if(!c)return"null";if(gap+=indent,u=[],"[object Array]"===Object.p'
			$g___OO_LoadLibrary &= 'rototype.toString.apply(c)){for(a=c.length,r=0;r<a;r+=1)u[r]=str(r,c)||"null";return o=0===u.length?'
			$g___OO_LoadLibrary &= '"[]":gap?"[\n"+gap+u.join(",\n"+gap)+"\n"+p+"]":"["+u.join(",")+"]",gap=p,o}if(rep&&"object"==typeof'
			$g___OO_LoadLibrary &= ' rep)for(a=rep.length,r=0;r<a;r+=1)"string"==typeof rep[r]&&(o=str(n=rep[r],c))&&u.push(quote(n)+(ga'
			$g___OO_LoadLibrary &= 'p?": ":":")+o);else for(n in c)Object.prototype.hasOwnProperty.call(c,n)&&(o=str(n,c))&&u.push(quote'
			$g___OO_LoadLibrary &= '(n)+(gap?": ":":")+o);return o=0===u.length?"{}":gap?"{\n"+gap+u.join(",\n"+gap)+"\n"+p+"}":"{"+u.jo'
			$g___OO_LoadLibrary &= 'in(",")+"}",gap=p,o}}"function"!=typeof Date.prototype.toJSON&&(Date.prototype.toJSON=function(){ret'
			$g___OO_LoadLibrary &= 'urn isFinite(this.valueOf())?this.getUTCFullYear()+"-"+f(this.getUTCMonth()+1)+"-"+f(this.getUTCDate'
			$g___OO_LoadLibrary &= '())+"T"+f(this.getUTCHours())+":"+f(this.getUTCMinutes())+":"+f(this.getUTCSeconds())+"Z":null},Stri'
			$g___OO_LoadLibrary &= 'ng.prototype.toJSON=Number.prototype.toJSON=Boolean.prototype.toJSON=function(){return this.valueOf('
			$g___OO_LoadLibrary &= ')});var cx,escapable,gap,indent,meta,rep;"function"!=typeof JSON.stringify&&(escapable=/[\\\"\x00-\x'
			$g___OO_LoadLibrary &= '1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-'
			$g___OO_LoadLibrary &= '\uffff]/g,meta={"\b":"\\b","\t":"\\t","\n":"\\n","\f":"\\f","\r":"\\r",''"'':''\\"'',"\\":"\\\\"},JS'
			$g___OO_LoadLibrary &= 'ON.stringify=function(t,e,r){var n;if(gap="",indent="","number"==typeof r)for(n=0;n<r;n+=1)indent+="'
			$g___OO_LoadLibrary &= ' ";else"string"==typeof r&&(indent=r);if(rep=e,e&&"function"!=typeof e&&("object"!=typeof e||"number'
			$g___OO_LoadLibrary &= '"!=typeof e.length))throw new Error("JSON.stringify");return str("",{"":t})}),"function"!=typeof JSO'
			$g___OO_LoadLibrary &= 'N.parse&&(cx=/[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-'
			$g___OO_LoadLibrary &= '\u206f\ufeff\ufff0-\uffff]/g,JSON.parse=function(text,reviver){function walk(t,e){var r,n,o=t[e];if(o&&"objec'
			$g___OO_LoadLibrary &= 't"==typeof o)for(r in o)Object.prototype.hasOwnProperty.call(o,r)&&(void 0!==(n=walk(o,r))?o[r]=n:de'
			$g___OO_LoadLibrary &= 'lete o[r]);return reviver.call(t,e,o)}var j;if(text=String(text),cx.lastIndex=0,cx.test(text)&&(text'
			$g___OO_LoadLibrary &= '=text.replace(cx,function(t){return"\\u"+("0000"+t.charCodeAt(0).toString(16)).slice(-4)})),/^[\],:{'
			$g___OO_LoadLibrary &= '}\s]*$/.test(text.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|fal'
			$g___OO_LoadLibrary &= 'se|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]").replace(/(?:^|:|,)(?:\s*\[)+/g,"")))return j=eval("'
			$g___OO_LoadLibrary &= '("+text+")"),"function"==typeof reviver?walk({"":j},""):j;throw new SyntaxError("JSON.parse")})}(),O'
			$g___OO_LoadLibrary &= 'bject.prototype.stringify=function(){return JSON.stringify(this)},REM="Print out an object  ",Object'
			$g___OO_LoadLibrary &= '.prototype.parse=function(t){return JSON.parse(t)},REM="JSON String to object  ",Object.prototype.js'
			$g___OO_LoadLibrary &= 'onPath=function(t,e){return JSON.jsonPath(this,t,e)},REM="Query object  ",Object.prototype.objToStri'
			$g___OO_LoadLibrary &= 'ng=function(){return JSON.stringify(this)},REM="Print out an object  - depreciated ",Object.prototyp'
			$g___OO_LoadLibrary &= 'e.strToObject=function(t){return JSON.parse(t)},REM="JSON String to object - deprecciated ";'
		EndIf
		Local $iError = 0, $oRet
		$g___OO_Json = ObjCreate('ScriptControl')
		If @error Then
			$iError = 1 + 0 * __HttpRequest_ErrNotify('_JSON_Object_Init', 'CreateObject Failed')
		Else
			With $g___OO_Json
				.language = 'Jscript'
				If @error Then
					$iError = 2 + 0 * __HttpRequest_ErrNotify('_JSON_Object_Init', 'EmbedJscript Failed')
				Else
					.AddCode($g___OO_LoadLibrary)
					If @error Then
						$iError = 3 + 0 * __HttpRequest_ErrNotify('_JSON_Object_Init', 'AddCode Failed')
					Else
						.Eval('var z = ' & $jsonDefault)
						$oRet = .Eval('z').objGet('JSON')
						If @error Then
							$iError = 4 + 0 * __HttpRequest_ErrNotify('_JSON_Object_Init', 'GetObject Failed')
						Else
							$g___OO_JsonInit = 1
						EndIf
					EndIf
...
#Region <INTERNAL FUNCTIONS>
	...
	Func __HttpRequest_URLSplit($sURL)
		...
		EndIf
		;---------------------------------------------------
		Local $aURL2 = StringRegExp($aURL1[3], '^(?:(\w*):)?(?:(\w*)(?:\:(\w*))?@)?(.+)$', 3) ;Tách user, pass, cred, URL
		...
		EndIf
		;---------------------------------------------------
		Local $aURL3 = StringRegExp($aURL2[3], '^([^\/\:]+)(?::(\d+))?(/.*)?($)', 3) ;Tách Host, (Port) và URI
		If @error Then Return SetError(4, __HttpRequest_ErrNotify('$sURL', 'Wrong Pattern #3'), '')
		$aResult[2] = $aURL1[2] & $aURL3[0] ;Host
